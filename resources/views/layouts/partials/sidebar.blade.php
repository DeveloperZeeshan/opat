            ...
            <div class="user-profile">
                <div class="dropdown user-pro-body ">
                    <div class="profile-image">
                        ...
                        @else
                            <img src="{{asset('website/'.auth()->user()->profile->pic)}}" alt="user-img" class="img-circle">
                        @endif
                        ...
                        <ul class="dropdown-menu animated flipInY">
                            <li><a href="{{url('account-settings')}}"><i class="fa fa-user"></i> Profile</a></li>
                    ...
                    <p class="profile-text m-t-15 font-16"><a href="javascript:void(0);"> @if(auth()->user()->name=="User") Admin @else {{auth()->user()->name}} @endif</a></p>
                </div>
            </div>
        @endif
            ...
            <ul id="side-menu">
                ...
                @if(auth()->user()->isAdmin() == true)
                    ...
                    @if(count(collect($section->items)) > 0)
                        ...
                        <li class="two-column">
                            <a class="waves-effect" href="javascript:void(0);" aria-expanded="false"><i
                                        class="icon-arrow-down-circle fa-fw"></i> <span class="hide-menu">CMS</span></a>
                            <ul aria-expanded="false" class="collapse">
                                <li><a href="{{asset('page/page')}}">Pages</a></li>
                                <li><a href="{{asset('ourSolution/our-solution')}}">Our Solutions</a></li>
                                <li><a href="{{asset('affordableCare/affordable-care')}}">Affordable Care</a></li>
                                <li><a href="{{asset('providesYourCare/provides-your-care')}}">Provides Your Care</a></li>
                                <li><a href="{{asset('resource/resource')}}">Community Resource Section</a></li>
                                <li><a href="{{asset('leadership/leadership')}}">Leadership</a></li>
                                <li><a href="{{asset('newsAndEvent/news-and-event')}}">News and Events</a></li>
                                <li><a href="{{asset('fAQ/f-a-q')}}">FAQ's</a></li>
                                <li><a href="{{asset('learningManagement/learning-management')}}">Resource</a></li>

                            </ul>
                        </li>
                        @endif
                            @foreach($section->items as $menu)
                                @if( $menu->title  =='Quiz' && auth()->user()->hasrole('consumer')) @continue @endif
                                @if( $menu->title  =='CognitivePsycological') @continue @endif
                                @if( $menu->title  =='SocialServiceEligibility') @continue @endif
                                @if( $menu->title  =='Page') @continue @endif
                                @if( $menu->title  =='OurSolution') @continue @endif
                                @if( $menu->title  =='AffordableCare') @continue @endif
                                @if( $menu->title  =='ProvidesYourCare') @continue @endif
                                @if( $menu->title  =='Resource') @continue @endif
                                @if( $menu->title  =='Leadership') @continue @endif
                                @if( $menu->title  =='NewsAndEvent') @continue @endif
                                @if( $menu->title  =='FAQ') @continue @endif
                                @if( $menu->title  =='LearningManagement') @continue @endif
                                @if(auth()->user()->hasrole('company'))
                                        @if( $menu->title  =='Transport Tracker') @continue @endif
                                @endif
                                     ...
                                     <li>
                                        <a class="waves-effect" href="{{ url($menu->url) }}">
                                            ...
                                            <span class="hide-menu">
                                                @if( $menu->title  =='EducationLevel')
                                                    Education Level
                                                @elseif( $menu->title  =='JobType')
                                                    Job Type
                                                @elseif( $menu->title  =='Consumer')
                                                    Resident
                                                @elseif( $menu->title  =='JobRequest')
                                                    Job Request
                                                @elseif( $menu->title  =='RentPayment')
                                                    Income Tracker
                                                @elseif( $menu->title  =='IncidentReport')
                                                    Incident Report
                                                @else
                                                    {{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', $menu->title) }}
                                                @endif
                                            </span>
                ...
                <li>
                    <a class="waves-effect" href="{{ url('calendar','all') }}">
