...
<div style="padding: 30px 0;">
    <table style=" width: 100%; margin-bottom: 35px;">
        ...
        <tbody>
        @foreach($data['incidentReport'] as $key=>$item)
            <tr style=" background: #eee;">
                <td style=" padding: 10px; color: black; text-align: right; font-size: 20px;">{{++$key}}</td>
                <td style=" padding: 10px; color: black; text-align: right; font-size: 20px;">{{$item['nature_of_incident']??''}}</td>
                <td style=" padding: 10px; color: black; text-align: right; font-size: 20px;">{{$item['incident_detail']??''}}</td>
                <td style=" padding: 10px; color: black; text-align: right; font-size: 20px;">{{$item['additional_notes']??''}}</td>
                <td style=" padding: 10px; color: black; text-align: right; font-size: 20px;">{{$item['incident_dates']??''}}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
